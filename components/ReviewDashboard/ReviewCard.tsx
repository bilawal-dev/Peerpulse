...
interface Props {
    review: { employeeId:number; name: string; manager: string; status: string; }
...
export default function ReviewCard({ review, reviewCycleId }: Props) {
    return (
        <div className="review-card bg-gray-50 border border-gray-200 rounded-lg p-4 sm:p-6 transform hover:-translate-y-1 hover:shadow-md transition">
            ...
            <Link href={`/admin/dashboard/${reviewCycleId}/compiled-reviews/${review.employeeId}`} className="view-button block w-fit bg-brand hover:bg-brand/90 text-white px-4 py-2 rounded-md text-sm font-medium">
                View Review
            </Link>
        </div>
    );
